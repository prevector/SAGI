...
import { registerUser, requestTask, submitSignal, getSignalResult, getWallet, } from "../sdk";
import { combatantFromCandidate } from "./combatantFromCandidate";
...
const readNum = (k) => Number(localStorage.getItem(k) ?? 0) || 0;
export function useGameSession() {
    ...
    const finish = useCallback((r) => {
        ...
        if (userId)
            getWallet(userId).then((w) => mountedRef.current && setWallet(w)).catch(() => { });
    }, []);
    const placeBet = useCallback(async (side) => {
        const userId = userIdRef.current;
        if (!userId || !task || phase !== "betting")
            return;
        setPicked(side);
        setPhase("fighting");
        try {
