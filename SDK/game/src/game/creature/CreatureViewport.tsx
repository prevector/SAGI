// Trimmed copy of apps/web/src/frontends/gene-terminal/CreatureViewport.tsx.
// Kept: CreatureWalker + the Canvas-less CreatureActor3D export. Dropped: the
// Canvas-mounting CreatureViewport export (its only uses of Canvas/OrbitControls/
// PCFShadowMap/CSS-module are gone with it). EvolutionGene + the morphology helpers
// come from the local ./creatureLibrary; TrainingStatus is inlined below.
...
function CreatureWalker({
  ...
  useEffect(() => {
    ...
    // Match the source: only re-pose on spec / ground changes. worldPosition and
    // worldHeading are read live in useFrame, so they don't belong in the deps
    // (an inline-array worldPosition would otherwise re-init the pose every render).
  ...
  useFrame((state, delta) => {
    ...
    const constraintIterations = status === "running" ? 5 : 3;
