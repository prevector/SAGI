...
from Tools import save_fig, get_data_path
...
class MazeTurnEnvVec:
    ...
    def step(self, actions):
        ...
        current_cells = self.mazes[np.arange(self.mazes.shape[0]), self.agents_pos[:, 0], self.agents_pos[:, 1]]
        self.agents_pos[current_cells==1] = pos_copy[current_cells==1]
        ...
        self._reset_food(self.consumed_count==1, prob=0.5)
        ...
        self.obs = self._get_obs_from_pos()
        ...
        return self.obs, self.rewards, dones, None
    ...
    def reset(self):
        ...
        self.random_swap_matrix = np.random.uniform(0, 1, size=(self.n_pseudo_env, max_eat)) >= 0.5
        self.random_swap_matrix = np.repeat(self.random_swap_matrix, int(self.n_agents/self.n_pseudo_env), axis=0)
    ...
    def render(self):
        ...
        max_render = 4
        flattened_render = np.dstack(np.split(self.mazes[:max_render, :], max_render, axis=0)).reshape(self.mazes.shape[1], -1)
        flattened_render[flattened_render>1] = 0
        plt.imshow(flattened_render)
        for i in range(max_render):
            ...
            if self.agent_energy[i]<=0:
                alpha = 0.4
            plt.scatter(self.agents_pos[i, 1] + i * self.mazes.shape[1], self.agents_pos[i, 0], color="skyblue", alpha=alpha, marker=marker)
            plt.scatter(self.agents_pos[i, 1] + i * self.mazes.shape[1], self.agents_pos[i, 0], color=obs_color,alpha=alpha, marker=marker, s=3)
            plt.scatter(self.food_pos[i, 0, 1] + i * self.mazes.shape[1], self.food_pos[i, 0, 0], color="lightgreen", alpha=0.6, marker="o")
            plt.scatter(self.food_pos[i, 1, 1] + i * self.mazes.shape[1], self.food_pos[i, 1, 0], color="red", alpha=0.6, marker="o")
        plt.pause(0.001)
        #plt.pause(2)
