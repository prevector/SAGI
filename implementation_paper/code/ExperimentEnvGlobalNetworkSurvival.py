...
class ExperimentEnvGlobalNetworkSurvival:
    ...
    def step(self, y):
        # NOTE: by permuting the meaning of each action the network should not be able exploit fact that of both 1 0 is picked
        y = np.take_along_axis(y, self.action_shuffle, axis=1)
        # take only 1 action every x ms, such that information has time to propagate and network doesn't have to try learn NOOP action
        self.y_hist += y
        if self.t % 4 != 0:
            actions = np.zeros((self.n_agents), dtype=np.int32) - 1
        else:
            # winner take all, in given time window
            actions = np.argmax(self.y_hist, axis=-1)
